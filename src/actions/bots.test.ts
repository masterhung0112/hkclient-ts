...
describe('Actions.Bots', () => {
  ...
  it('loadBots', async () => {
    ...
    nock(Client4.getBaseRoute()).get('/bots').query(true).reply(201, bots)
  ...
  it('loadBot', async () => {
    ...
    nock(Client4.getBaseRoute()).get(`/bots/${bot.user_id}`).query(true).reply(201, bot)
  ...
  it('createBot', async () => {
    const bot = TestHelper.fakeBot()
    nock(Client4.getBaseRoute()).post('/bots').reply(200, bot)
  ...
  it('patchBot', async () => {
    ...
    nock(Client4.getBaseRoute()).put(`/bots/${bot.user_id}`).reply(200, bot)
  ...
  it('disableBot', async () => {
    ...
    nock(Client4.getBotRoute(bot.user_id)).post('/disable').reply(200, bot)
    ...
    bot.delete_at = 0
    nock(Client4.getBotRoute(bot.user_id)).post('/enable').reply(200, bot)
  ...
  it('logout', async () => {
    ...
    nock(Client4.getUsersRoute()).post('/logout').reply(200, { status: 'OK' })

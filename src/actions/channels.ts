...
export function createDirectChannel(userId: string, otherUserId: string): ActionFunc {
  return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
    ...
    try {
      ...
      forceLogoutIfNecessary(error, dispatch, getState)
      dispatch(batchActions([{ type: ChannelTypes.CREATE_CHANNEL_FAILURE, error }, logError(error)]))
      return { error }
    }
...
export function patchChannel(channelId: string, patch: Channel): ActionFunc {
  return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
    ...
    try {
      ...
      dispatch(batchActions([{ type: ChannelTypes.UPDATE_CHANNEL_FAILURE, error }, logError(error)]))
      return { error }
    }
    dispatch(
      batchActions([
  ...
  return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
    ...
    if (viewTime < new Date().getTime() - 3 * 60 * 60 * 1000) {
      const preferences = [
        {
          user_id: currentUserId,
          category: Preferences.CATEGORY_CHANNEL_APPROXIMATE_VIEW_TIME,
          name: channelId,
          value: new Date().getTime().toString(),
        },
      ]
...
export function markChannelAsViewed(channelId: string, prevChannelId = '') {
  return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
    const actions: Action[] = []
...
export function getChannels(teamId: string, page = 0, perPage: number = General.CHANNELS_CHUNK_SIZE): ActionFunc {
  return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
    ...
    try {
      ...
      forceLogoutIfNecessary(error, dispatch, getState)
      dispatch(batchActions([{ type: ChannelTypes.GET_CHANNELS_FAILURE, error }, logError(error)]))
      return { error }
    }
...
export function getAllChannelsWithCount(
  ...
  return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
    ...
    try {
      ...
      forceLogoutIfNecessary(error, dispatch, getState)
      dispatch(batchActions([{ type: ChannelTypes.GET_ALL_CHANNELS_FAILURE, error }, logError(error)]))
      return { error }
    }
...
export function markChannelAsUnread(
  teamId: string,
  channelId: string,
  mentions: string[],

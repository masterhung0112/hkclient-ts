    ...
    return sdk
      ...
      .then((json: any) => {
        if (json.errorMessage) {
          ...
          Client4.trackEvent('gfycat', 'views', { context, count: json.gfycats.length, keyword: searchText })
        }
      })
      .catch((err: any) => dispatch(errorSearching(err, searchText)))
  }
}
    ...
    return gfycatSdk(GfycatApiKey!, GfycatApiSecret!)
      ...
      .then((json: any) => {
        if (json.errorMessage) {
          ...
          Client4.trackEvent('gfycat', 'views', { context: 'category', count: json.gfycats.length, keyword: tagName })
...
export function requestCategoriesListIfNeeded({ count } = { count: undefined }) {
  return (dispatch: DispatchFunc, getState: GetStateFunc) => {
    const state = getState().entities.gifs.categories

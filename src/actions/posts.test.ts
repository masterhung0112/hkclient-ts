...
describe('Actions.Posts', () => {
  ...
  it('maintain postReplies', async () => {
    ...
    nock(Client4.getBaseRoute()).post('/posts').reply(201, post2)
    ...
    nock(Client4.getBaseRoute()).delete(`/posts/${post2.id}`).reply(200, OK_RESPONSE)
  ...
  it('resetCreatePostRequest', async () => {
    ...
    nock(Client4.getBaseRoute()).post('/posts').reply(400, createPostError)
  ...
  it('createPost with file attachments', async () => {
    ...
    await Actions.createPost(post, files)(store.dispatch, store.getState)
  ...
  it('editPost', async () => {
    ...
    nock(Client4.getBaseRoute()).post('/posts').reply(201, TestHelper.fakePostWithId(channelId))

    const post = await Client4.createPost(TestHelper.fakePost(channelId))
    const message = post.message

    post.message = `${message} (edited)`

    nock(Client4.getBaseRoute()).put(`/posts/${post.id}/patch`).reply(200, post)

    await Actions.editPost(post)(store.dispatch, store.getState)
    ...
    assert.strictEqual(posts[post.id].message, `${message} (edited)`)
  })
  ...
  it('deletePost', async () => {
    ...
    nock(Client4.getBaseRoute()).delete(`/posts/${postId}`).reply(200, OK_RESPONSE)
    ...
    assert.strictEqual(posts[postId].state, Posts.POST_DELETED)
  })
  ...
  it('deletePostWithReaction', async () => {
    ...
    nock(Client4.getBaseRoute()).post('/posts').reply(201, TestHelper.fakePostWithId(TestHelper.basicChannel.id))

    const post1 = await Client4.createPost(TestHelper.fakePost(TestHelper.basicChannel.id))

    const emojiName = '+1'

    nock(Client4.getBaseRoute()).post('/reactions').reply(201, {
      user_id: TestHelper.basicUser.id,
      post_id: post1.id,
      emoji_name: emojiName,
      create_at: 1508168444721,
    })
    ...
    nock(Client4.getBaseRoute()).delete(`/posts/${post1.id}`).reply(200, OK_RESPONSE)
  ...
  it('getPostsUnread', async () => {
    ...
    nock(Client4.getUsersRoute()).get(`/${userId}/channels/${channelId}/posts/unread`).query(true).reply(200, response)
  ...
  it('getPosts', async () => {
    ...
    nock(Client4.getChannelsRoute()).get('/channel1/posts').query(true).reply(200, postList)
  ...
  it('getPostsBefore', async () => {
    ...
    nock(Client4.getChannelsRoute()).get(`/${channelId}/posts`).query(true).reply(200, postList)
  ...
  it('getPostsAround', async () => {
    ...
    nock(Client4.getBaseRoute()).get(`/posts/${postId}/thread`).query(true).reply(200, postsThread)
    ...
    expect(result.data).toEqual({
      ...
      order: [...postsAfter.order, postId, ...postsBefore.order],
    ...
    expect(postsInChannel[channelId]).toEqual([
      { order: ['post1', 'post2', 'post3', 'post4', 'post5'], recent: false, oldest: false },
    ])
  ...
  it('flagPost', async () => {
    ...
    nock(Client4.getUsersRoute()).post('/logout').reply(200, OK_RESPONSE)
    ...
    const post1 = await Client4.createPost(TestHelper.fakePost(channelId))
  ...
  it('unflagPost', async () => {
    ...
    nock(Client4.getUsersRoute()).delete(`/${TestHelper.basicUser.id}/preferences`).reply(200, OK_RESPONSE)
  ...
  it('pinPost', async () => {
    ...
    nock(Client4.getBaseRoute()).post(`/posts/${post1.id}/pin`).reply(200, OK_RESPONSE)
  ...
  it('unpinPost', async () => {
    ...
    nock(Client4.getBaseRoute()).post(`/posts/${post1.id}/unpin`).reply(200, OK_RESPONSE)
  ...
  it('getReactionsForPost', async () => {
    ...
    nock(Client4.getBaseRoute())
      ...
      .reply(200, [
        { user_id: TestHelper.basicUser.id, post_id: post1.id, emoji_name: emojiName, create_at: 1508168444721 },
      ])
  ...
  it('getOpenGraphMetadata', async () => {
    ...
    nock(Client4.getBaseRoute()).post('/opengraph').reply(200, {
      type: 'article',
      url: 'https://about.mattermost.com/',
      title: 'Mattermost private cloud messaging',
      description: 'Open source,  private cloud\nSlack-alternative, \nWorkplace messaging for web, PCs and phones.',
    })
    await dispatch(Actions.getOpenGraphMetadata(url))

    nock(Client4.getBaseRoute()).post('/opengraph').reply(200, { type: '', url: '', title: '', description: '' })
    await dispatch(Actions.getOpenGraphMetadata(docs))

    nock(Client4.getBaseRoute()).post('/opengraph').reply(200, null)
    await dispatch(Actions.getOpenGraphMetadata(docs))
  ...
  it('doPostAction', async () => {
    ...
    const { data } = await Actions.doPostAction(
      'posth67ja7ntdkek6g13dp3wka',
      'action7ja7ntdkek6g13dp3wka',
      'option'
  ...
  it('doPostActionWithCookie', async () => {
    ...
    const { data } = await Actions.doPostActionWithCookie(
      'posth67ja7ntdkek6g13dp3wka',
      'action7ja7ntdkek6g13dp3wka',
      '',
      'option'
  ...
  describe('getThreadsForPosts', () => {
    ...
    beforeEach(async () => {
      ...
      store.dispatch(
        Actions.receivedPostsInChannel(
          {
    ...
    it('handlesNull', async () => {
      ...
      const { postsInChannel, postsInThread } = state.entities.posts

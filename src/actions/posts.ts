...
export function getPost(postId: string) {
  return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
    ...
    try {
      ...
      dispatch(batchActions([{ type: PostTypes.GET_POSTS_FAILURE, error }, logError(error)]))
      return { error }
    }
...
export function pinPost(postId: string) {
  return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
    ...
    try {
      ...
      forceLogoutIfNecessary(error, dispatch, getState)
      dispatch(batchActions([{ type: PostTypes.EDIT_POST_FAILURE, error }, logError(error)]))
      return { error }
    }
...
export function getPostThread(rootId: string, fetchThreads = true) {
  return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
    ...
    try {
      ...
      forceLogoutIfNecessary(error, dispatch, getState)
      dispatch(batchActions([{ type: PostTypes.GET_POST_THREAD_FAILURE, error }, logError(error)]))
      return { error }
    }
...
export function getPostsBefore(
  ...
  return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
    ...
    dispatch(
      batchActions([receivedPosts(posts), receivedPostsBefore(posts, channelId, postId, posts.prev_post_id === '')])
    )
...
export function getPostsAfter(
  ...
  return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
    ...
    dispatch(
      batchActions([receivedPosts(posts), receivedPostsAfter(posts, channelId, postId, posts.next_post_id === '')])
    )
...
export function getProfilesAndStatusesForPosts(
  postsArrayOrMap: Post[] | Map<string, Post>,
...
export function getNeededCustomEmojis(state: GlobalState, posts: Post[]): Set<string> {
  ...
  posts.forEach((post) => {
    if (post.message.includes(':')) {
      ...
      const emojisFromPost = parseNeededCustomEmojisFromText(
        post.message,
        systemEmojis,
        customEmojisByName,
        nonExistentEmoji
    ...
    if (props && props.attachments && props.attachments.length) {
      ...
      if (attachmentText) {
        const emojisFromAttachment = parseNeededCustomEmojisFromText(
          attachmentText,
          systemEmojis,
          customEmojisByName,
          nonExistentEmoji
...
export function resetHistoryIndex(index: number | string) {
  return async (dispatch: DispatchFunc) => {
    dispatch({
...
export function moveHistoryIndexBack(index: number | string) {
  return async (dispatch: DispatchFunc) => {
    dispatch({
...
export function moveHistoryIndexForward(index: number | string) {
  return async (dispatch: DispatchFunc) => {
    dispatch({

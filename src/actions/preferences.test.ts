...
describe('Actions.Preferences', () => {
  ...
  it('deletePreferences', async () => {
    ...
    nock(Client4.getUsersRoute()).post(`/${TestHelper.basicUser.id}/preferences/delete`).reply(200, OK_RESPONSE)
    await Actions.deletePreferences(user.id, [existingPreferences[0], existingPreferences[2]])(
      store.dispatch,
      store.getState
    )
  ...
  it('deleteTeamSpecificThemes', async () => {
    ...
    nock(Client4.getUsersRoute()).put(`/${user.id}/preferences`).reply(200, OK_RESPONSE)
    await Client4.savePreferences(user.id, existingPreferences)

    nock(Client4.getUsersRoute()).get('/me/preferences').reply(200, existingPreferences)
    await Actions.getMyPreferences()(store.dispatch, store.getState)

    nock(Client4.getUsersRoute()).post(`/${user.id}/preferences/delete`).reply(200, OK_RESPONSE)

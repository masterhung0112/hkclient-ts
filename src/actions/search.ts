...
export function searchFiles(teamId: string, terms: string, isOrSearch: boolean, includeDeletedChannels: boolean) {
  return searchFilesWithParams(teamId, {
    terms,
    is_or_search: isOrSearch,
    include_deleted_channels: includeDeletedChannels,
    page: 0,
...
export function getFlaggedPosts(): ActionFunc {
  return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
    ...
    try {
      ...
      await Promise.all([
        getProfilesAndStatusesForPosts(posts.posts, dispatch, getState) as any,
        dispatch(getMissingChannelsFromPosts(posts.posts)) as any,
      ])
    } catch (error) {
      forceLogoutIfNecessary(error, dispatch, getState)
      dispatch(batchActions([{ type: SearchTypes.SEARCH_FLAGGED_POSTS_FAILURE, error }, logError(error)]))
      return { error }
    }
...
export function getPinnedPosts(channelId: string): ActionFunc {
  return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
    ...
    try {
      ...
      forceLogoutIfNecessary(error, dispatch, getState)
      dispatch(batchActions([{ type: SearchTypes.SEARCH_PINNED_POSTS_FAILURE, error }, logError(error)]))
      return { error }
    }

...
export function searchTeams(term: string, opts: TeamSearchOpts = {}): ActionFunc {
  return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
    ...
    try {
      ...
      forceLogoutIfNecessary(error, dispatch, getState)
      dispatch(batchActions([{ type: TeamTypes.GET_TEAMS_FAILURE, error }, logError(error)]))
      return { error }
    }
...
export function joinTeam(inviteId: string, teamId: string): ActionFunc {
  return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
    ...
    try {
      ...
      forceLogoutIfNecessary(error, dispatch, getState)
      dispatch(batchActions([{ type: TeamTypes.JOIN_TEAM_FAILURE, error }, logError(error)]))
      return { error }
    }

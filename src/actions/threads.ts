...
export function getThreads(
  ...
  return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
    ...
    try {
      userThreadList = await Client4.getUserThreads(userId, teamId, {
        before,
        after,
        pageSize: perPage,
        extended: true,
    ...
    if (userThreadList?.threads?.length) {
      dispatch({
        ...
        data: userThreadList.threads
          .map(({ participants }) => participants.filter((user) => user.id !== entities.users.currentUserId))
          .flat(),
      })

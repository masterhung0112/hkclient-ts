...
export function checkMfa(loginId: string): ActionFunc {
  return async (dispatch: DispatchFunc) => {
    ...
    try {
      ...
      dispatch(batchActions([{ type: UserTypes.CHECK_MFA_FAILURE, error }, logError(error)]))
      return { error }
    }
  }
}
    ...
    for (let i = 0; i < values.length; i++) {
      ...
      if (member) {
        if (member.mention_count > 0 && isDirectChannel(channel)) {
          ...
          !isGroupChannelVisible(config, myPreferences, channel)
        ) {
...
export function searchProfiles(term: string, options: any = {}): ActionFunc {
  return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
    ...
    const actions: Action[] = [
      { type: UserTypes.RECEIVED_PROFILES_LIST, data: removeUserFromList(currentUserId, [...profiles]) },
    ]
...
export function updateMe(user: UserProfile): ActionFunc {
  return async (dispatch: DispatchFunc) => {
    ...
    dispatch(batchActions([{ type: UserTypes.RECEIVED_ME, data }, { type: UserTypes.UPDATE_ME_SUCCESS }]))

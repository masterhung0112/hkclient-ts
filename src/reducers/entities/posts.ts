  ...
  switch (action.type) {
    ...
    case PostTypes.RECEIVED_POSTS_IN_CHANNEL: {
      ...
      if (recent) {
        ...
        if (recentBlockIndex !== -1) {
          ...
          if (
            recentBlock.order.length === order.length &&
    ...
    case PostTypes.POST_REMOVED: {
      ...
      for (let i = 0; i < nextPostsForChannel.length; i++) {
        ...
        const nextOrder = block.order.filter(
          (postId: string) => postId !== post.id && prevPosts[postId].root_id !== post.id
        )
...
function storeOpenGraphForPost(state: any, post: Post) {
  ...
  return post.metadata.embeds.reduce((nextState, embed) => {
    ...
    const postIdState = nextState[post.id]
      ? { ...nextState[post.id], [embed.url]: embed.data }
...
export default function reducer(state: PostsState = {} as PostsState, action: GenericAction): PostsState {
  ...
  if (
    ...
    state.postsInChannel === nextState.postsInChannel &&

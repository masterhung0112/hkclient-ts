...
import { AdminTypes, ChannelTypes, TeamTypes, UserTypes, SchemeTypes, GroupTypes } from 'action-types'
...
function myMembers(state: RelationOneToOne<Team, TeamMembership> = {}, action: GenericAction) {
  function updateState(
    receivedTeams: IDMappedObjects<Team> = {},
    currentState: RelationOneToOne<Team, TeamMembership> = {}
  ) {
  ...
  switch (action.type) {
    ...
    case ChannelTypes.POST_UNREAD_SUCCESS: {
      ...
      const newTeamState = {
        ...
        mention_count:
          typeof teamState.mention_count === 'undefined' ? mentionCount : teamState.mention_count + mentionCount,
      }
...
function membersInTeam(
  state: RelationOneToOne<Team, RelationOneToOne<UserProfile, TeamMembership>> = {},
...
function groupsAssociatedToTeam(
  state: RelationOneToOne<Team, { ids: string[]; totalCount: number }> = {},
  action: GenericAction
) {
  switch (action.type) {

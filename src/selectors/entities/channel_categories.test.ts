...
describe('getCategoryInTeamByType', () => {
  ...
  test('should return categories from each team', () => {
    ...
    expect(Selectors.getCategoryInTeamByType(state, 'team1', CategoryTypes.DIRECT_MESSAGES)).toBe(
      directMessagesCategory1
    )
...
describe('legacyMakeFilterAutoclosedDMs', () => {
  ...
  const baseState = {
    entities: {
      ...
      preferences: {
        myPreferences: {
          [getPreferenceKey(Preferences.CATEGORY_SIDEBAR_SETTINGS, Preferences.CHANNEL_SIDEBAR_AUTOCLOSE_DMS)]: {
            value: Preferences.AUTOCLOSE_DMS_ENABLED,
          },
        },
      },
      users: {
  ...
  test('should hide an inactive GM channel', () => {
    ...
    const gmChannel: any = { id: 'gmChannel', type: General.GM_CHANNEL }
      ...
      entities: {
        preferences: {
          myPreferences: {
            ...
            [getPreferenceKey(Preferences.CATEGORY_CHANNEL_APPROXIMATE_VIEW_TIME, gmChannel.id)]: {
              value: `${cutoff - 1}`,
            },
          },
        },
  ...
  test('should show a GM channel if it had an unloaded post made recently', () => {
    ...
    const gmChannel: any = { id: 'gmChannel', type: General.GM_CHANNEL, last_post_at: cutoff + 1 }
  ...
  test('should show a GM channel if it has unread messages', () => {
    ...
    const gmChannel: any = { id: 'gmChannel', type: General.GM_CHANNEL, total_msg_count: 1 }
  ...
  test('should hide an inactive DM channel', () => {
    ...
    const dmChannel: any = { id: 'dmChannel', name: `${currentUser.id}__${otherUser.id}`, type: General.DM_CHANNEL }
      ...
      entities: {
        preferences: {
          myPreferences: {
            ...
            [getPreferenceKey(Preferences.CATEGORY_CHANNEL_APPROXIMATE_VIEW_TIME, dmChannel.id)]: {
              value: `${cutoff - 1}`,
            },
          },
        },
  ...
  test('should return the original array when no items are removed', () => {
    ...
    const channel1: any = { id: 'channel1', type: General.PUBLIC_CHANNEL }

    const state: any = baseState

    const channels: any = [channel1]
...
describe('makeFilterAutoclosedDMs', () => {
  ...
  test('Should always show an unread channel', () => {
    ...
    const gmChannel1: any = { id: 'gmChannel1', type: General.GM_CHANNEL, total_msg_count: 5 }
    ...
    const state = mergeObjects(baseState, {
      entities: {
        ...
        preferences: {
          myPreferences: {
            [getPreferenceKey(Preferences.CATEGORY_SIDEBAR_SETTINGS, Preferences.LIMIT_VISIBLE_DMS_GMS)]: {
              value: '1',
            },
          },
        },
    ...
    expect(filterAutoclosedDMs(state, [gmChannel1, gmChannel2] as any, CategoryTypes.DIRECT_MESSAGES)).toEqual([
      gmChannel1,
    ] as any)
  })
  ...
  test('Should always show the current channel', () => {
    ...
    const dmChannel1: any = { id: 'dmChannel1', type: General.DM_CHANNEL, name: `${currentUser.id}__${jeffWinger.id}` }
    const gmChannel1: any = { id: 'gmChannel1', type: General.GM_CHANNEL }
  ...
  describe('Should always show the exact number of channels specified by the user', () => {
    ...
    const dmChannel1: any = { id: 'dmChannel1', type: General.DM_CHANNEL, name: `${currentUser.id}__${tigerKing.id}` }
    const gmChannel1: any = { id: 'gmChannel1', type: General.GM_CHANNEL, name: 'WhatsApp' }
    ...
    test('User specified 5 DMs to be shown', () => {
      ...
      expect(
        filterAutoclosedDMs(
          state,
          [dmChannel1, gmChannel1, gmChannel2, dmChannel2, dmChannel3],
          CategoryTypes.DIRECT_MESSAGES
        )
      ).toEqual([dmChannel1, gmChannel1, gmChannel2, dmChannel2, dmChannel3])
    })
  ...
  test('should consider approximate view time and open time preferences for most recently viewed channel', () => {
    ...
    expect(filterAutoclosedDMs(state, [dmChannel1, dmChannel2, dmChannel3], CategoryTypes.DIRECT_MESSAGES)).toEqual([
      dmChannel1,
      dmChannel2,
    ])
...
describe('makeFilterManuallyClosedDMs', () => {
  ...
  test('should filter DMs based on preferences', () => {
    ...
    const dmChannel1: any = { id: 'dmChannel1', type: General.DM_CHANNEL, name: `${currentUser.id}__${otherUser1.id}` }
  ...
  test('should show unread DMs and GMs, regardless of preferences', () => {
    ...
    const gmChannel1: any = { id: 'gmChannel1', type: General.GM_CHANNEL, total_msg_count: 1 }
    ...
    expect(filterManuallyClosedDMs(state, [dmChannel1, dmChannel2, gmChannel1, gmChannel2])).toEqual([
      dmChannel1,
      gmChannel1,
    ])
  })
  ...
  test('should not filter other channels', () => {
    ...
    const channel1: any = { id: 'channel1', type: General.OPEN_CHANNEL }
    const channel2: any = { id: 'channel2', type: General.PRIVATE_CHANNEL }
...
describe('makeSortChannelsByName', () => {
  ...
  test('should sort channels by display name', () => {
    ...
    const channel1: any = { id: 'channel1', display_name: 'Carrot' }
    const channel2: any = { id: 'channel2', display_name: 'Apple' }
  ...
  test('should sort channels by display name with numbers', () => {
    ...
    const channel1: any = { id: 'channel1', display_name: 'Channel 10' }
    const channel2: any = { id: 'channel2', display_name: 'Channel 1' }
...
describe('makeSortChannelsByNameWithDMs', () => {
  ...
  const channel1: any = { id: 'channel1', type: General.OPEN_CHANNEL, display_name: 'Zebra' }
  const channel2: any = { id: 'channel2', type: General.PRIVATE_CHANNEL, display_name: 'Aardvark' }
  ...
  const baseState: any = {
    entities: {
      ...
      preferences: {
        myPreferences: {
          [getPreferenceKey(Preferences.CATEGORY_DISPLAY_SETTINGS, Preferences.NAME_NAME_FORMAT)]: {
  ...
  test('should sort regular channels by display name', () => {
    ...
    expect(sortChannelsByNameWithDMs(baseState, [channel1, channel2, channel3])).toMatchObject([
      channel2, // Aardvark
      channel3, // Bear
      channel1, // Zebra
    ])
  ...
  test('should sort DM channels by the display name of the other user', () => {
    ...
    expect(sortChannelsByNameWithDMs(baseState, [channel1, channel2, channel3, dmChannel1, dmChannel2])).toMatchObject([
      channel2, // Aardvark
  ...
  test('should sort GM channels by the display name of the other users', () => {
    ...
    expect(sortChannelsByNameWithDMs(state, [channel1, channel2, channel3, gmChannel1])).toMatchObject([
      channel2, // Aardvark
  ...
  test('should sort muted channels last', () => {
    ...
    expect(
      sortChannelsByNameWithDMs(state, [channel1, channel2, channel3, dmChannel1, dmChannel2, gmChannel1])
    ).toMatchObject([
      channel2, // Aardvark
      dmChannel2, // Another User
      channel1, // Zebra
...
describe('makeSortChannelsByRecency', () => {
  const channel1: any = { id: 'channel1', display_name: 'Apple', last_post_at: 1000 }
  const channel2: any = { id: 'channel2', display_name: 'Banana', last_post_at: 2000 }
...
describe('makeGetChannelsForCategory', () => {
  ...
  test('should return sorted and filtered channels for direct messages category with alphabetical sorting', () => {
    ...
    const directMessagesCategory: any = {
  ...
  test('should return sorted and filtered channels for direct messages category with recency sorting', () => {
    ...
    expect(getChannelsForCategory(state, directMessagesCategory)).toMatchObject([
      { id: 'dmChannel1' },
      { id: 'gmChannel2' },
      { id: 'gmChannel1' },
    ])
  ...
  const baseState: any = {
    entities: {
      channelCategories: {
        ...
        orderByTeam: {
          team1: [favoritesCategory.id, channelsCategory.id, directMessagesCategory.id],
        },
      },
      channels: {
        channels: {

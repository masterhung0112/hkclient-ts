...
import { getUserIdFromChannelName, isChannelMuted, isUnreadChannel } from 'utils/channel_utils'
...
export function getCategoryInTeamByType(state: GlobalState, teamId: string, categoryType: ChannelCategoryType) {
  return getCategoryWhere(state, (category) => category.type === categoryType && category.team_id === teamId)
}
...
export function getCategoryInTeamWithChannel(state: GlobalState, teamId: string, channelId: string) {
  return getCategoryWhere(state, (category) => category.team_id === teamId && category.channel_ids.includes(channelId))
}
  ...
  return createSelector(
    ...
    (channels: Channel[], currentChannelId: string) => {
      const filtered = channels.filter(
        (channel) => channel && (channel.id === currentChannelId || channel.delete_at === 0)
      )
...
export function legacyMakeFilterAutoclosedDMs(
  getAutocloseCutoff = getDefaultAutocloseCutoff
  ...
  return createSelector(
    ...
    (
      ...
      const filtered = channels.filter((channel) => {
        ...
        const viewTimePref =
          myPreferences[getPreferenceKey(Preferences.CATEGORY_CHANNEL_APPROXIMATE_VIEW_TIME, channel.id)]
...
export function makeCompareChannels(
  getDisplayName: (channel: Channel) => string,
  locale: string,

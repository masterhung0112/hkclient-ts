...
export const canDownloadFilesOnMobile: (a: GlobalState) => boolean = createSelector(
  ...
  (config: ClientConfig, license: any): boolean => {
    ...
    return (
      license.IsLicensed === 'false' || license.Compliance === 'false' || config.EnableMobileFileDownload !== 'false'
    )
  }
)
...
export const getManagedResourcePaths: (state: GlobalState) => string[] = createSelector(getConfig, (config) => {

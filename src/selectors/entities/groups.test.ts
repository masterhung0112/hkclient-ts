...
describe('Selectors.Groups', () => {
  ...
  it('getGroupsNotAssociatedToTeam', () => {
    const expected = Object.entries(testState.entities.groups.groups)
      .filter(([groupID]) => !teamAssociatedGroupIDs.includes(groupID))
      .map(([, group]) => group)
  ...
  it('getGroupsNotAssociatedToChannel', () => {
    let expected = Object.values(testState.entities.groups.groups).filter(
      (group: Group) => !channelAssociatedGroupIDs.includes(group.id)
    )
    ...
    expected = Object.values(cloneState.entities.groups.groups).filter(
      (group: Group) =>
        !channelAssociatedGroupIDs.includes(group.id) &&
        cloneState.entities.teams.groupsAssociatedToTeam[teamID].ids.includes(group.id)
    )

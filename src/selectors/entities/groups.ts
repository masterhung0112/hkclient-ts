...
export const getAssociatedGroupsByName: (
  ...
  teamID: string,
  channelId: string
) => NameMappedObjects<Group> = createSelector(getAssociatedGroupsForReference, (groups) => {
...
export const getAssociatedGroupsForReferenceByMention: (
  ...
  teamID: string,
  channelId: string
) => Map<string, Group> = createSelector(getAssociatedGroupsForReference, (groups) => {
...
const getTeamGroupIDSet = createSelector(teamGroupIDs, (teamIDs) => new Set(teamIDs))

const getChannelGroupIDSet = createSelector(channelGroupIDs, (channelIDs) => new Set(channelIDs))
...
export const getGroupsAssociatedToTeamForReference: (state: GlobalState, teamID: string) => Group[] = createSelector(
  ...
  (allGroups, teamGroupIDSet) => {
    return Object.entries(allGroups)
      .filter(([groupID]) => teamGroupIDSet.has(groupID))
      .filter((entry) => entry[1].allow_reference && entry[1].delete_at === 0)
      .map((entry) => entry[1])
  }
)
...
export const getMyAllowReferencedGroups: (state: GlobalState) => Group[] = createSelector(getMyGroups, (myGroups) => {
...
export const getMyGroupsAssociatedToChannelForReference: (
  ...
  teamId: string,
  channelId: string
) => Group[] = createSelector(getMyGroups, getAssociatedGroupsByName, (myGroups, groups) => {
...
export const getMyGroupMentionKeysForChannel: (
  ...
  teamId: string,
  channelId: string
) => UserMentionKey[] = createSelector(getMyGroupsAssociatedToChannelForReference, (groups: Group[]) => {
  const keys: UserMentionKey[] = []
  groups.forEach((group) => keys.push({ key: `@${group.name}` }))
  return keys
})

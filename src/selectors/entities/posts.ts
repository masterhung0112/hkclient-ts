...
import { MessageHistory, OpenGraphMetadata, Post, PostOrderBlock, PostWithFormatData } from 'types/posts'
...
export function makeGetPostIdsAroundPost(): (
  ...
  return createIdsSelector(
    ...
    (postsChunk, postId, postsBeforeCount = Posts.POST_CHUNK_SIZE / 2, postsAfterCount = Posts.POST_CHUNK_SIZE / 2) => {
      ...
      const maxPostIndex =
        postsBeforeCount === -1 ? postIds.length : Math.min(index + postsBeforeCount + 1, postIds.length) // Needs the extra 1 to include the focused post
...
export function makeGetPostsInChannel(): (
  ...
  return createSelector(
    ...
    (allPosts, postsInThread, allPostIds, currentUser, myPreferences, numPosts) => {
      ...
      const joinLeavePref =
        myPreferences[getPreferenceKey(Preferences.CATEGORY_ADVANCED_SETTINGS, Preferences.ADVANCED_FILTER_JOIN_LEAVE)]
...
export function makeGetPostsAroundPost(): (
  ...
  return createSelector(
    (state: GlobalState, focusedPostId: string, channelId: string) =>
      getPostIdsAroundPost(state, focusedPostId, channelId, options),
    getAllPosts,
    getPostsInThread,
    ...
    (postIds, allPosts, postsInThread, focusedPostId, currentUser, myPreferences) => {
      ...
      for (let i = 0; i < postIds.length; i++) {
        ...
        const previousPost = allPosts[postIds[i + 1]] || null
        const formattedPost = formatPostInChannel(
          post,
          previousPost,
          i,
          allPosts,
...
export function makeGetProfilesForThread(): (state: GlobalState, props: { rootId: $ID<Post> }) => UserProfile[] {
  ...
  return createSelector(getUsers, getCurrentUserId, getPostsForThread, (allUsers, currentUserId, posts) => {
    const profileIds = posts.map((post) => post.user_id)
    const uniqueIds = [...new Set(profileIds)]
    return uniqueIds.reduce((acc, id) => {
      if (allUsers[id] && currentUserId !== id) {
        return [...acc, allUsers[id]]
      }
      return acc
    }, [])
  })
...
export const getMostRecentPostIdInChannel: (
  state: GlobalState,
  channelId: $ID<Channel>
) => $ID<Post> | undefined | null = createSelector(
  getAllPosts,
...
export const getLatestReplyablePostId: (state: GlobalState) => $ID<Post> = createSelector(
  ...
  (posts) => {
    ...
    const latestReplyablePost = posts.find(
      (post) => post.state !== Posts.POST_DELETED && !isSystemMessage(post) && !isPostEphemeral(post)
    )
...
export function getRecentPostsChunkInChannel(
  state: GlobalState,
  channelId: $ID<Channel>
): PostOrderBlock | null | undefined {
...
export function getOldestPostsChunkInChannel(
  state: GlobalState,
  channelId: $ID<Channel>
): PostOrderBlock | null | undefined {
  const postsForChannel = state.entities.posts.postsInChannel[channelId]
...
export function getPostsChunkInChannelAroundTime(
  state: GlobalState,
  channelId: $ID<Channel>,
  timeStamp: number
): PostOrderBlock | undefined | null {

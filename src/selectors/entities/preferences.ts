...
export const getTeammateNameDisplaySetting: (state: GlobalState) => string | undefined = createSelector(
  ...
  (config, preferences, license) => {
    const useAdminTeammateNameDisplaySetting =
      license && license.LockTeammateNameDisplay === 'true' && config.LockTeammateNameDisplay === 'true'
...
const getThemePreference = createSelector(getMyPreferences, getCurrentTeamId, (myPreferences, currentTeamId) => {
...
export const getSidebarPreferences: (state: GlobalState) => SidebarPreferences = createSelector(
  (state: GlobalState) => {
    ...
    return (
      config.ExperimentalGroupUnreadChannels !== General.DISABLED &&
      getBool(
        state,
  ...
  (state) => {
    return get(state, Preferences.CATEGORY_SIDEBAR_SETTINGS, '', null)
  },
...
export function shouldAutocloseDMs(state: GlobalState) {
  ...
  const preference = get(
    state,
    Preferences.CATEGORY_SIDEBAR_SETTINGS,
    Preferences.CHANNEL_SIDEBAR_AUTOCLOSE_DMS,
    Preferences.AUTOCLOSE_DMS_ENABLED
  )
...
export function isCollapsedThreadsAllowed(state: GlobalState): boolean {
  return getFeatureFlagValue(state, 'CollapsedThreads') === 'true' && getConfig(state).CollapsedThreads !== 'disabled'
}
...
export function isCollapsedThreadsEnabled(state: GlobalState): boolean {
  ...
  return (
    ...
    (userPreference === Preferences.COLLAPSED_REPLY_THREADS_ON ||
      (getConfig(state).CollapsedThreads as string) === 'always_on')
  )
}

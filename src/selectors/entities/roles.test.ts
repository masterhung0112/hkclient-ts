...
describe('Selectors.Roles', () => {
  ...
  it('should return my system permission on getMySystemPermissions', () => {
    assert.deepEqual(getMySystemPermissions(testState), new Set(['user_role2']))
  })
  ...
  it('should return my team permission on getMyCurrentTeamPermissions', () => {
    assert.deepEqual(Selectors.getMyCurrentTeamPermissions(testState), new Set(['user_role2', 'team1_role1']))
  })
  ...
  it('should return if i have a channel permission on haveIChannelPermission', () => {
    assert.equal(
      Selectors.haveIChannelPermission(testState, { team: team1.id, channel: channel1.id, permission: 'user_role2' }),
      true
    )
    assert.equal(
      Selectors.haveIChannelPermission(testState, { team: team1.id, channel: channel1.id, permission: 'team1_role1' }),
      true
    )
    assert.equal(
      Selectors.haveIChannelPermission(testState, { team: team1.id, channel: channel1.id, permission: 'team2_role2' }),
      false
    )
    assert.equal(
      Selectors.haveIChannelPermission(testState, {
        team: team1.id,
        channel: channel1.id,
        permission: 'channel_a_role1',
      }),
      true
    )
    assert.equal(
  ...
  it('should return my current channel permission on getMyCurrentChannelPermissions', () => {
    assert.deepEqual(
      Selectors.getMyCurrentChannelPermissions(testState),
      new Set(['user_role2', 'team1_role1', 'channel_a_role1', 'channel_a_role2'])
    )
  })

...
describe('Selectors.Users', () => {
  ...
  myPreferences[`${Preferences.CATEGORY_DIRECT_CHANNEL_SHOW}--${user3.id}`] = {
    category: Preferences.CATEGORY_DIRECT_CHANNEL_SHOW,
    name: user3.id,
    value: 'false',
  }
  ...
  describe('getCurrentUserMentionKeys', () => {
    ...
    it('channel', () => {
      ...
      assert.deepEqual(Selectors.getCurrentUserMentionKeys(state), [
        { key: '@channel' },
        { key: '@all' },
        { key: '@here' },
        { key: '@user' },
  ...
  describe('getProfilesInTeam', () => {
    ...
    it('getProfilesInTeam with role filters', () => {
      assert.deepEqual(
        Selectors.getProfilesInTeam(testState, team1.id, { roles: ['system_admin'] }),
        [user1, user7].sort(sortByUsername)
      )
    ...
    it('getProfilesInTeam with multiple filters', () => {
      const users = [user7]
      assert.deepEqual(
        Selectors.getProfilesInTeam(testState, team1.id, { role: 'system_admin', inactive: true }),
        users
      )
    })
  })
  ...
  describe('searchProfilesStartingWithTerm', () => {
    ...
    it('searchProfiles with filters', () => {
      assert.deepEqual(searchProfilesStartingWithTerm(testState, user1.username, false, { role: 'system_admin' }), [
        user1,
      ])
      ...
      assert.deepEqual(searchProfilesStartingWithTerm(testState, user3.username, false, { roles: ['system_user'] }), [
        user3,
      ])
  ...
  describe('searchProfilesMatchingWithTerm', () => {
    it('searchProfiles without filter', () => {
      assert.deepEqual(searchProfilesMatchingWithTerm(testState, user1.username.slice(1, user1.username.length)), [
        user1,
      ])
    ...
    it('searchProfiles with filters', () => {
      assert.deepEqual(
        searchProfilesMatchingWithTerm(testState, user1.username.slice(2, user1.username.length), false, {
          role: 'system_admin',
        }),
        [user1]
      )
      assert.deepEqual(
        searchProfilesMatchingWithTerm(testState, user3.username.slice(3, user3.username.length), false, {
          role: 'system_admin',
        }),
        []
      )
      assert.deepEqual(
        searchProfilesMatchingWithTerm(testState, user1.username.slice(0, user1.username.length), false, {
          roles: ['system_user'],
        }),
        []
      )
      assert.deepEqual(searchProfilesMatchingWithTerm(testState, user3.username, false, { roles: ['system_user'] }), [
        user3,
      ])
  ...
  describe('searchProfilesInTeam', () => {
    ...
    it('searchProfilesInTeam with filter', () => {
      assert.deepEqual(
        Selectors.searchProfilesInTeam(testState, team1.id, user1.username, false, { role: 'system_admin' }),
        [user1]
      )
      assert.deepEqual(
        Selectors.searchProfilesInTeam(testState, team1.id, user1.username, false, { inactive: true }),
        []
      )
      assert.deepEqual(Selectors.searchProfilesInTeam(testState, team1.id, user2.username, false, { active: true }), [])
      assert.deepEqual(Selectors.searchProfilesInTeam(testState, team1.id, user1.username, false, { active: true }), [
        user1,
      ])
    })
    it('getProfiles with multiple filters', () => {
      const users = [user7]
      assert.deepEqual(
        Selectors.searchProfilesInTeam(testState, team1.id, user7.username, false, {
          role: 'system_admin',
          inactive: true,
        }),
        users
  ...
  describe('searchProfilesWithoutTeam', () => {
    ...
    it('searchProfilesWithoutTeam with filter', () => {
      assert.deepEqual(
        Selectors.searchProfilesWithoutTeam(testState, user6.username, false, { role: 'system_admin' }),
        [user6]
      )
  ...
  it('makeGetProfilesInChannel', () => {
    ...
    assert.deepEqual(
      getProfilesInChannel(testState, channel2.id, { channel_roles: ['channel_admin', 'channel_user'] }),
      [user2]
    )
    assert.deepEqual(
      getProfilesInChannel(testState, channel2.id, {
        roles: ['system_admin'],
        channel_roles: ['channel_admin', 'channel_user'],
      }),
      [user1, user2].sort(sortByUsername)
    )
  ...
  it('makeGetProfilesNotInChannel', () => {
    ...
    assert.deepEqual(
      getProfilesNotInChannel(testState, channel2.id, { active: true }),
      [user4, user5].sort(sortByUsername)
    )
    ...
    assert.deepEqual(
      getProfilesNotInChannel(testState, channel2.id, { role: 'system_user' }),
      [user4, user5].sort(sortByUsername)
    )
  ...
  it('makeGetProfilesByIdsAndUsernames', () => {
    ...
    const testCases = [
      ...
      {
        input: { allUserIds: [user1.id, user2.id], allUsernames: [user3.username, user4.username] },
  ...
  it('shouldShowTermsOfService', () => {
    ...
    assert.equal(
      Selectors.shouldShowTermsOfService({
        entities: {
          general: {
            config: {

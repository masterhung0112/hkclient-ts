...
export const isCurrentUserSystemAdmin: (state: GlobalState) => boolean = createSelector(getCurrentUser, (user) => {
  const roles = user?.roles || ''
  return isSystemAdmin(roles)
})

export const currentUserHasAnAdminRole: (state: GlobalState) => boolean = createSelector(getCurrentUser, (user) => {
  const roles = user.roles || ''
...
export function filterProfiles(
  profiles: IDMappedObjects<UserProfile>,
  filters?: Filters,
  memberships?: RelationOneToOne<UserProfile, TeamMembership> | RelationOneToOne<UserProfile, ChannelMembership>
...
export const shouldShowTermsOfService: (state: GlobalState) => boolean = createSelector(
  ...
  (config, user, license) => {
    ...
    return Boolean(
      user && featureEnabled && (config.CustomTermsOfServiceId !== acceptedTermsId || timeElapsed > reacceptanceTime)
    )
  }
)
...
export function makeGetProfilesByIdsAndUsernames(): (
  ...
  return createSelector(
    getUsers,
    getUsersByUsername,
    (state: GlobalState, props: { allUserIds: Array<$ID<UserProfile>>; allUsernames: Array<$Username<UserProfile>> }) =>
      props.allUserIds,
    (state, props) => props.allUsernames,
    (
      allProfilesById: Dictionary<UserProfile>,
      allProfilesByUsername: Dictionary<UserProfile>,
      allUserIds: string[],
      allUsernames: string[]
    ) => {

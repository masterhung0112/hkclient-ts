...
import { Scheme, SchemeScope } from 'types/schemes'
...
class TestHelper {
  basicClient4: Client4
  basicUser: UserProfile
  basicTeam: Team
  basicTeamMember: TeamMembership
  basicChannel: Channel
  basicChannelMember: ChannelMembership
  basicPost: Post
  basicRoles: Record<any, Role>
  basicScheme: Scheme
  basicGroup: Group
  ...
  fakeTeamMember = (userId, teamId): TeamMembership => {
    return {
      user_id: userId,
      team_id: teamId,
      roles: 'team_user',
  ...
  fakeDmChannel = (userId, otherUserId): Channel => {
    return {
  ...
  fakeChannelMember = (userId, channelId): ChannelMembership => {
    return {
      user_id: userId,
  ...
  mockLogin = () => {
    ...
    nock(this.basicClient4.getBaseRoute()).get('/users/me/teams/members').reply(200, [this.basicTeamMember])

    nock(this.basicClient4.getBaseRoute())
      .get('/users/me/teams/unread')
      .reply(200, [{ team_id: this.basicTeam.id, msg_count: 0, mention_count: 0 }])

    nock(this.basicClient4.getBaseRoute()).get('/users/me/teams').reply(200, [this.basicTeam])
  ...
  initMockEntities = () => {
    ...
    this.basicGroup = this.fakeGroupWithId(this.generateId())
  }

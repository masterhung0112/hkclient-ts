...
describe('ChannelUtils', () => {
  it('canManageMembersOldPermissions', () => {
    ...
    const townSquareChannel: any = { name: General.DEFAULT_CHANNEL, type: General.OPEN_CHANNEL }
    const publicChannel: any = { type: General.PUBLIC_CHANNEL }
    const privateChannel: any = { type: General.PRIVATE_CHANNEL }
    const gmChannel: any = { type: General.GM_CHANNEL }
    const dmChannel: any = { type: General.DM_CHANNEL }

    const systemAdmin: any = { roles: General.SYSTEM_USER_ROLE + ' ' + General.SYSTEM_ADMIN_ROLE }
    const systemUser: any = { roles: General.SYSTEM_USER_ROLE }

    const teamAdmin: any = { roles: General.TEAM_USER_ROLE + ' ' + General.TEAM_ADMIN_ROLE }
    const teamUser: any = { roles: General.TEAM_USER_ROLE }

    const channelAdmin: any = { roles: General.CHANNEL_USER_ROLE + ' ' + General.CHANNEL_ADMIN_ROLE }
    const channelUser: any = { roles: General.CHANNEL_USER_ROLE }
    ...
    assert.ok(
      !canManageMembersOldPermissions(dmChannel, systemAdmin, teamAdmin, channelAdmin, anyoneCanManageMembers, licensed)
    )
    assert.ok(
      !canManageMembersOldPermissions(
        gmChannel,
    ...
    assert.ok(
      !canManageMembersOldPermissions(gmChannel, systemAdmin, teamAdmin, channelAdmin, anyoneCanManageMembers, licensed)
    )
    ...
    assert.ok(
      canManageMembersOldPermissions(publicChannel, systemUser, teamUser, channelUser, anyoneCanManageMembers, licensed)
    )
    assert.ok(
      canManageMembersOldPermissions(
        publicChannel,
  ...
  it('isAutoClosed', () => {
    const autoCloseEnabled: any = { CloseUnusedDirectMessages: 'true' }
    const autoCloseDisabled: any = { CloseUnusedDirectMessages: 'false' }
    const activeChannel: any = { id: 'channelid', last_post_at: new Date().getTime() }
    const inactiveChannel: any = { id: 'channelid', last_post_at: 1 }
  ...
  it('areChannelMentionsIgnored', () => {
    const currentUserNotifyProps1: any = { channel: 'true' }
    const channelMemberNotifyProps1: any = { ignore_channel_mentions: Users.IGNORE_CHANNEL_MENTIONS_DEFAULT }
    assert.equal(false, areChannelMentionsIgnored(channelMemberNotifyProps1, currentUserNotifyProps1))

    const currentUserNotifyProps2: any = { channel: 'true' }
    const channelMemberNotifyProps2: any = { ignore_channel_mentions: Users.IGNORE_CHANNEL_MENTIONS_OFF }
    assert.equal(false, areChannelMentionsIgnored(channelMemberNotifyProps2, currentUserNotifyProps2))

    const currentUserNotifyProps3: any = { channel: 'true' }
    const channelMemberNotifyProps3: any = { ignore_channel_mentions: Users.IGNORE_CHANNEL_MENTIONS_ON }
    assert.equal(true, areChannelMentionsIgnored(channelMemberNotifyProps3, currentUserNotifyProps3))

    const currentUserNotifyProps4: any = { channel: 'false' }
    const channelMemberNotifyProps4: any = { ignore_channel_mentions: Users.IGNORE_CHANNEL_MENTIONS_DEFAULT }
    assert.equal(true, areChannelMentionsIgnored(channelMemberNotifyProps4, currentUserNotifyProps4))

    const currentUserNotifyProps5: any = { channel: 'false' }
    const channelMemberNotifyProps5: any = { ignore_channel_mentions: Users.IGNORE_CHANNEL_MENTIONS_OFF }
    assert.equal(false, areChannelMentionsIgnored(channelMemberNotifyProps5, currentUserNotifyProps5))

    const currentUserNotifyProps6: any = { channel: 'false' }
    const channelMemberNotifyProps6: any = { ignore_channel_mentions: Users.IGNORE_CHANNEL_MENTIONS_ON }
    assert.equal(true, areChannelMentionsIgnored(channelMemberNotifyProps6, currentUserNotifyProps6))

    const currentUserNotifyProps7: any = { channel: true }
    const channelMemberNotifyProps7: any = null
    assert.equal(false, areChannelMentionsIgnored(channelMemberNotifyProps7, currentUserNotifyProps7))

    const currentUserNotifyProps8: any = { channel: false }
    const channelMemberNotifyProps8: any = null
  ...
  it('sortChannelsByRecency', () => {
    ...
    assert.deepEqual(
      sortChannelsByRecency(lastPosts, channelA, channelB),
      2,
  ...
  it('sortChannelsByTypeListAndDisplayName', () => {
    ...
    let actual: any = [channelOpen1, channelPrivate, channelDM, channelGM, channelOpen2].sort(sortfn)
    let expected: any = [channelOpen1, channelOpen2, channelPrivate, channelDM, channelGM]

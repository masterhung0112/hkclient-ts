...
export function isAutoClosed(
  ...
  const autoClose =
    myPreferences[getPreferenceKey(Preferences.CATEGORY_SIDEBAR_SETTINGS, Preferences.CHANNEL_SIDEBAR_AUTOCLOSE_DMS)]
...
export function isGroupOrDirectChannelVisible(
  ...
  if (
    ...
    isGroupChannelVisible(config, myPreferences, channel, lastPost, isUnreadChannel(memberships, channel), now)
  ) {
    return true
  }
...
export function showCreateOption(
  state: GlobalState,
  config: any,
  license: any,
  teamId: string,
...
export function showManagementOptions(
  state: GlobalState,
  config: any,
  license: any,
  channel: Channel,
...
export function showDeleteOption(
  state: GlobalState,
  config: any,
  license: any,
  channel: Channel,
  ...
  if (hasNewPermissions(state)) {
    if (channel.type === General.OPEN_CHANNEL) {
      return haveIChannelPermission(state, {
        channel: channel.id,
        team: channel.team_id,
        permission: Permissions.DELETE_PUBLIC_CHANNEL,
      })
    } else if (channel.type === General.PRIVATE_CHANNEL) {
      return haveIChannelPermission(state, {
        channel: channel.id,
        team: channel.team_id,
        permission: Permissions.DELETE_PRIVATE_CHANNEL,
      })
    }
    return true
  }
...
export function canManageMembersOldPermissions(
  channel: Channel,
  user: UserProfile,
  ...
  if (channel.type === General.PRIVATE_CHANNEL) {
    ...
    if (
      config.RestrictPrivateChannelManageMembers === General.PERMISSIONS_TEAM_ADMIN &&
      !isTeamAdmin &&
      !isSystemAdmin
    ) {
      return false
    ...
    if (
      config.RestrictPrivateChannelManageMembers === General.PERMISSIONS_CHANNEL_ADMIN &&
      !isChannelAdmin &&
      !isTeamAdmin &&
      !isSystemAdmin
    ) {
...
export function isFavoriteChannelOld(
  myPreferences: {
    [x: string]: PreferenceType
  },
  id: string

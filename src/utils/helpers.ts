...
export function isValidUsername(name: string): string {
  ...
  } else if (name.length < General.MIN_USERNAME_LENGTH || name.length > General.MAX_USERNAME_LENGTH) {
    error = 'Must be between ' + General.MIN_USERNAME_LENGTH + ' and ' + General.MAX_USERNAME_LENGTH + ' characters'
...
export function isValidPassword(
  ...
  let valid = true
  const minimumLength = passwordConfig.minimumLength || General.MIN_PASSWORD_LENGTH

  if (password.length < minimumLength || password.length > General.MAX_PASSWORD_LENGTH) {
    valid = false
  }

...
describe('makeFilterPostsAndAddSeparators', () => {
  it('filter join/leave posts', () => {
    ...
    assert.deepEqual(now, ['1002', '1001', 'date-' + today.getTime()])
    ...
    now = filterPostsAndAddSeparators(state, { postIds, lastViewedAt, indicateNewMessages })
    assert.deepEqual(now, ['1001', 'date-' + today.getTime()])
  ...
  it('new messages indicator', () => {
    ...
    assert.deepEqual(now, ['1010', '1005', '1000', 'date-' + (today.getTime() + 1000)])

    now = filterPostsAndAddSeparators(state, { postIds, indicateNewMessages: true } as any)
    assert.deepEqual(now, ['1010', '1005', '1000', 'date-' + (today.getTime() + 1000)])
    ...
    assert.deepEqual(now, ['1010', '1005', '1000', START_OF_NEW_MESSAGES, 'date-' + (today.getTime() + 1000)])
    ...
    assert.deepEqual(now, ['1010', '1005', START_OF_NEW_MESSAGES, '1000', 'date-' + (today.getTime() + 1000)])

    now = filterPostsAndAddSeparators(state, { postIds, lastViewedAt: time + 1006, indicateNewMessages: true })
    assert.deepEqual(now, ['1010', START_OF_NEW_MESSAGES, '1005', '1000', 'date-' + (today.getTime() + 1000)])

    // Don't show indicator when all posts are read
    now = filterPostsAndAddSeparators(state, { postIds, lastViewedAt: time + 1020 } as any)
    assert.deepEqual(now, ['1010', '1005', '1000', 'date-' + (today.getTime() + 1000)])
  })
...
describe('makeCombineUserActivityPosts', () => {
  ...
  test('should "combine" a single activity post', () => {
    ...
    expect(result).not.toBe(postIds)
    expect(result).toEqual(['post1', COMBINED_USER_ACTIVITY + 'post2', 'post3'])
  })

  test('should not combine with regular messages', () => {
    const combineUserActivityPosts = makeCombineUserActivityPosts()

    const postIds = deepFreeze(['post1', 'post2', 'post3', 'post4', 'post5'])
    const state: any = {
      entities: {
        posts: {
          posts: {
    ...
    expect(result).not.toBe(postIds)
    expect(result).toEqual([COMBINED_USER_ACTIVITY + 'post1_post2', 'post3', COMBINED_USER_ACTIVITY + 'post4_post5'])
  })
  ...
  test('should not combine with other system messages', () => {
    ...
    expect(result).not.toBe(postIds)
    expect(result).toEqual([COMBINED_USER_ACTIVITY + 'post1', 'post2', COMBINED_USER_ACTIVITY + 'post3'])
  })

  test('should not combine across non-post items', () => {
    const combineUserActivityPosts = makeCombineUserActivityPosts()

    const postIds = deepFreeze(['post1', START_OF_NEW_MESSAGES, 'post2', 'post3', DATE_LINE + '1001', 'post4'])
    const state: any = {
      entities: {
        posts: {
          posts: {
  ...
  describe('memoization', () => {
    const initialPostIds: any = ['post1', 'post2']
...
describe('getPostIdsForCombinedUserActivityPost', () => {
  test('should get IDs correctly', () => {
    expect(getPostIdsForCombinedUserActivityPost('user-activity-post1_post2_post3')).toEqual([
      'post1',
      'post2',
      'post3',
    ])
...
describe('combineUserActivitySystemPost', () => {
  ...
  const postAddToChannel1: any = {
    type: PostTypes.ADD_TO_CHANNEL,
    user_id: 'user_id_1',
    props: { addedUserId: 'added_user_id_1', addedUsername: 'added_username_1' },
  }
  const postAddToChannel2: any = {
    type: PostTypes.ADD_TO_CHANNEL,
    user_id: 'user_id_1',
    props: { addedUserId: 'added_user_id_2', addedUsername: 'added_username_2' },
  }
  const postAddToChannel3: any = {
    type: PostTypes.ADD_TO_CHANNEL,
    user_id: 'user_id_1',
    props: { addedUserId: 'added_user_id_3', addedUsername: 'added_username_3' },
  }
  const postAddToChannel4: any = {
    type: PostTypes.ADD_TO_CHANNEL,
    user_id: 'user_id_2',
    props: { addedUserId: 'added_user_id_4', addedUsername: 'added_username_4' },
  }
  const postAddToChannel5: any = {
    type: PostTypes.ADD_TO_CHANNEL,
    user_id: 'user_id_1',
    props: { addedUsername: 'added_username_1' },
  }
  it('should match return for ADD_TO_CHANNEL', () => {
      ...
      messageData: [
        {
          ...
          userIds: ['added_user_id_1', 'added_user_id_2', 'added_user_id_3'],
        },
      ],
    }
    ...
    assert.deepEqual(
      combineUserActivitySystemPost([postAddToChannel1, postAddToChannel2, postAddToChannel3, postAddToChannel4]),
      out4
    )
      ...
      messageData: [
        {
          actorId: 'user_id_1',
          postType: PostTypes.ADD_TO_CHANNEL,
          userIds: ['added_username_1', 'added_user_id_1', 'added_user_id_2', 'added_user_id_3'],
        },
        { actorId: 'user_id_2', postType: PostTypes.ADD_TO_CHANNEL, userIds: ['added_user_id_4'] },
      ],
    }
  ...
  it('should match return for ADD_TO_CHANNEL, backward compatibility with addedUsername', () => {
    ...
    assert.deepEqual(
      combineUserActivitySystemPost([{ ...postAddToChannel1, props: { addedUsername: 'added_user_name_1' } }]),
      out1
    )
    ...
    const out2 = {
      ...
      allUsernames: ['added_user_name_1'],
      messageData: [
        { actorId: 'user_id_1', postType: PostTypes.ADD_TO_CHANNEL, userIds: ['added_user_name_1', 'added_user_id_2'] },
      ],
    }
    assert.deepEqual(
      combineUserActivitySystemPost([
        { ...postAddToChannel1, props: { addedUsername: 'added_user_name_1' } },
        postAddToChannel2,
      ]),
      out2
    )
    ...
    assert.deepEqual(
      combineUserActivitySystemPost([
        ...
        postAddToChannel2,
        postAddToChannel3,
        { ...postAddToChannel4, props: { addedUsername: 'added_user_name_4' } },
      ]),
      out3
    )
  })

  const postAddToTeam1: any = {
    type: PostTypes.ADD_TO_TEAM,
    user_id: 'user_id_1',
    props: { addedUserId: 'added_user_id_1' },
  }
  const postAddToTeam2: any = {
    type: PostTypes.ADD_TO_TEAM,
    user_id: 'user_id_1',
    props: { addedUserId: 'added_user_id_2' },
  }
  const postAddToTeam3: any = {
    type: PostTypes.ADD_TO_TEAM,
    user_id: 'user_id_1',
    props: { addedUserId: 'added_user_id_3' },
  }
  const postAddToTeam4: any = {
    type: PostTypes.ADD_TO_TEAM,
    user_id: 'user_id_2',
    props: { addedUserId: 'added_user_id_4' },
  }
  it('should match return for ADD_TO_TEAM', () => {
    ...
    assert.deepEqual(
      combineUserActivitySystemPost([postAddToTeam1, postAddToTeam2, postAddToTeam3, postAddToTeam4]),
      out4
    )
  })
  ...
  it('should match return for ADD_TO_TEAM, backward compatibility with addedUsername', () => {
    ...
    assert.deepEqual(
      combineUserActivitySystemPost([{ ...postAddToTeam1, props: { addedUsername: 'added_user_name_1' } }]),
      out1
    )
    ...
    const out2 = {
      ...
      allUsernames: ['added_user_name_1'],
      messageData: [
        { actorId: 'user_id_1', postType: PostTypes.ADD_TO_TEAM, userIds: ['added_user_name_1', 'added_user_id_2'] },
      ],
    }
    assert.deepEqual(
      combineUserActivitySystemPost([
        { ...postAddToTeam1, props: { addedUsername: 'added_user_name_1' } },
        postAddToTeam2,
      ]),
      out2
    )
    ...
    assert.deepEqual(
      combineUserActivitySystemPost([
        ...
        postAddToTeam2,
        postAddToTeam3,
        { ...postAddToTeam4, props: { addedUsername: 'added_user_name_4' } },
      ]),
      out3
    )
  })

  const postJoinChannel1: any = { type: PostTypes.JOIN_CHANNEL, user_id: 'user_id_1' }
  const postJoinChannel2: any = { type: PostTypes.JOIN_CHANNEL, user_id: 'user_id_2' }
  const postJoinChannel3: any = { type: PostTypes.JOIN_CHANNEL, user_id: 'user_id_3' }
  const postJoinChannel4: any = { type: PostTypes.JOIN_CHANNEL, user_id: 'user_id_4' }
  it('should match return for JOIN_CHANNEL', () => {
    ...
    assert.deepEqual(
      combineUserActivitySystemPost([postJoinChannel1, postJoinChannel2, postJoinChannel3, postJoinChannel4]),
      out4
    )
  })

  const postJoinTeam1: any = { type: PostTypes.JOIN_TEAM, user_id: 'user_id_1' }
  const postJoinTeam2: any = { type: PostTypes.JOIN_TEAM, user_id: 'user_id_2' }
  const postJoinTeam3: any = { type: PostTypes.JOIN_TEAM, user_id: 'user_id_3' }
  const postJoinTeam4: any = { type: PostTypes.JOIN_TEAM, user_id: 'user_id_4' }
  ...
  const postLeaveChannel1: any = { type: PostTypes.LEAVE_CHANNEL, user_id: 'user_id_1' }
  const postLeaveChannel2: any = { type: PostTypes.LEAVE_CHANNEL, user_id: 'user_id_2' }
  const postLeaveChannel3: any = { type: PostTypes.LEAVE_CHANNEL, user_id: 'user_id_3' }
  const postLeaveChannel4: any = { type: PostTypes.LEAVE_CHANNEL, user_id: 'user_id_4' }
  it('should match return for LEAVE_CHANNEL', () => {
    ...
    assert.deepEqual(
      combineUserActivitySystemPost([postLeaveChannel1, postLeaveChannel2, postLeaveChannel3, postLeaveChannel4]),
      out4
    )
  })

  const postLeaveTeam1: any = { type: PostTypes.LEAVE_TEAM, user_id: 'user_id_1' }
  const postLeaveTeam2: any = { type: PostTypes.LEAVE_TEAM, user_id: 'user_id_2' }
  const postLeaveTeam3: any = { type: PostTypes.LEAVE_TEAM, user_id: 'user_id_3' }
  const postLeaveTeam4: any = { type: PostTypes.LEAVE_TEAM, user_id: 'user_id_4' }
  it('should match return for LEAVE_TEAM', () => {
    ...
    assert.deepEqual(
      combineUserActivitySystemPost([postLeaveTeam1, postLeaveTeam2, postLeaveTeam3, postLeaveTeam4]),
      out4
    )
  })

  const postRemoveFromChannel1: any = {
    type: PostTypes.REMOVE_FROM_CHANNEL,
    user_id: 'user_id_1',
    props: { removedUserId: 'removed_user_id_1' },
  }
  const postRemoveFromChannel2: any = {
    type: PostTypes.REMOVE_FROM_CHANNEL,
    user_id: 'user_id_1',
    props: { removedUserId: 'removed_user_id_2' },
  }
  const postRemoveFromChannel3: any = {
    type: PostTypes.REMOVE_FROM_CHANNEL,
    user_id: 'user_id_1',
    props: { removedUserId: 'removed_user_id_3' },
  }
  const postRemoveFromChannel4: any = {
    type: PostTypes.REMOVE_FROM_CHANNEL,
    user_id: 'user_id_1',
    props: { removedUserId: 'removed_user_id_4' },
  }
      ...
      messageData: [
        {
          actorId: 'user_id_1',
          postType: PostTypes.REMOVE_FROM_CHANNEL,
          userIds: ['removed_user_id_1', 'removed_user_id_2', 'removed_user_id_3'],
        },
      ],
    }
    assert.deepEqual(
      combineUserActivitySystemPost([postRemoveFromChannel1, postRemoveFromChannel2, postRemoveFromChannel3]),
      out3
    )
      ...
      messageData: [
        {
          actorId: 'user_id_1',
          postType: PostTypes.REMOVE_FROM_CHANNEL,
          userIds: ['removed_user_id_1', 'removed_user_id_2', 'removed_user_id_3', 'removed_user_id_4'],
        },
      ],
    }
    assert.deepEqual(
  ...
  const postRemoveFromTeam1: any = { type: PostTypes.REMOVE_FROM_TEAM, user_id: 'user_id_1' }
  const postRemoveFromTeam2: any = { type: PostTypes.REMOVE_FROM_TEAM, user_id: 'user_id_2' }
  const postRemoveFromTeam3: any = { type: PostTypes.REMOVE_FROM_TEAM, user_id: 'user_id_3' }
  const postRemoveFromTeam4: any = { type: PostTypes.REMOVE_FROM_TEAM, user_id: 'user_id_4' }
  it('should match return for REMOVE_FROM_TEAM', () => {
    ...
    assert.deepEqual(
      combineUserActivitySystemPost([postRemoveFromTeam1, postRemoveFromTeam2, postRemoveFromTeam3]),
      out3
    )
    ...
    const out4 = {
      ...
      allUsernames: [],
      messageData: [
        { postType: PostTypes.REMOVE_FROM_TEAM, userIds: ['user_id_1', 'user_id_2', 'user_id_3', 'user_id_4'] },
      ],
    }
    assert.deepEqual(
      combineUserActivitySystemPost([
  ...
  it('should match return on combination', () => {
    ...
    assert.deepEqual(
      combineUserActivitySystemPost([postAddToChannel1, postAddToChannel2, postAddToTeam1, postAddToTeam2]),
      out1
    )
    ...
    assert.deepEqual(
      combineUserActivitySystemPost([postJoinChannel1, postJoinChannel2, postJoinTeam1, postJoinTeam2]),
      out2
    )
    ...
    assert.deepEqual(
      combineUserActivitySystemPost([postLeaveChannel1, postLeaveChannel2, postLeaveTeam1, postLeaveTeam2]),
      out3
    )
    ...
    const out7 = {
      ...
      messageData: [
        { postType: PostTypes.JOIN_TEAM, userIds: ['user_id_3', 'user_id_4', 'user_id_1', 'user_id_2'] },
        {
          actorId: 'user_id_1',
          postType: PostTypes.ADD_TO_TEAM,
          userIds: ['added_user_id_3', 'added_user_id_1', 'added_user_id_2'],
        },
        { actorId: 'user_id_2', postType: PostTypes.ADD_TO_TEAM, userIds: ['added_user_id_4'] },
...
describe('comparePostTypes', () => {
  ...
  const testCases = [
    ...
    [
      { postType: JOIN_CHANNEL },
      { postType: ADD_TO_CHANNEL },
      { postType: LEAVE_CHANNEL },
      { postType: REMOVE_FROM_CHANNEL },
    ],
    [
      { postType: REMOVE_FROM_CHANNEL },
      { postType: LEAVE_CHANNEL },
  ...
  it('should sort post type correctly', () => {
    for (const testCase of testCases) {
      ...
      testCase.sort(comparePostTypes as any).forEach((sortedTestCase, index) => {
        if (index > 0) {
          assert.ok(
            postTypePriority[previousType] <= postTypePriority[sortedTestCase.postType],
            `${previousType} should come first before ${sortedTestCase.postType}`
          )
        }

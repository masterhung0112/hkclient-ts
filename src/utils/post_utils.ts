...
export function canDeletePost(
  ...
  if (hasNewPermissions(state)) {
    ...
    if (!isOwner) {
      return (
        canDelete &&
        haveIChannelPermission(state, { team: teamId, channel: channelId, permission: Permissions.DELETE_OTHERS_POSTS })
      )
    }
    return canDelete
  }
  ...
  if (license.IsLicensed === 'true') {
    return (
      (config.RestrictPostDelete === General.PERMISSIONS_ALL && (isOwner || isAdmin)) ||

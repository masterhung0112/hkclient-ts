...
describe('user utils', () => {
  describe('displayUsername', () => {
    ...
    it('should return username when no nick name and no full name', () => {
      assert.equal(
        displayUsername(
          { ...userObj, nickname: '', first_name: '', last_name: '' },
    ...
    it('should return username when no full name', () => {
      assert.equal(
        displayUsername({ ...userObj, first_name: '', last_name: '' }, Preferences.DISPLAY_PREFER_FULL_NAME),
        'testUser'
      )
    })
  ...
  describe('Utils.applyRolesFilters', () => {
    ...
    const adminUser = {
      ...
      roles: `${General.SYSTEM_USER_ROLE} ${General.SYSTEM_ADMIN_ROLE}`,
    }
    ...
    it('Non admin user with non admin membership', () => {
      const nonAdminMembership = {
        ...TestHelper.fakeTeamMember(nonAdminUser.id, team.id),
        scheme_admin: false,
        scheme_user: true,
      }
      ...
      assert.equal(
        applyRolesFilters(
          nonAdminUser,
          [General.SYSTEM_ADMIN_ROLE, General.TEAM_ADMIN_ROLE, General.CHANNEL_ADMIN_ROLE],
          [],
          nonAdminMembership
        ),
        false
      ...
      assert.equal(
        applyRolesFilters(nonAdminUser, [General.TEAM_USER_ROLE], [General.SYSTEM_ADMIN_ROLE], nonAdminMembership),
        true
      )
      assert.equal(
        applyRolesFilters(nonAdminUser, [General.TEAM_ADMIN_ROLE], [General.SYSTEM_ADMIN_ROLE], nonAdminMembership),
        false
      )
      assert.equal(
        applyRolesFilters(nonAdminUser, [General.TEAM_USER_ROLE], [General.SYSTEM_USER_ROLE], nonAdminMembership),
        false
      )
    })
    ...
    it('Non admin user with admin membership', () => {
      ...
      assert.equal(
        applyRolesFilters(
          nonAdminUser,
          [General.SYSTEM_ADMIN_ROLE, General.TEAM_USER_ROLE, General.CHANNEL_USER_ROLE],
          [],
          adminMembership
        ),
        false
      ...
      assert.equal(
        applyRolesFilters(nonAdminUser, [General.SYSTEM_USER_ROLE], [General.CHANNEL_ADMIN_ROLE], adminMembership),
        false
      )
    })
    ...
    it('Admin user with any membership', () => {
      const nonAdminMembership = {
        ...TestHelper.fakeTeamMember(adminUser.id, team.id),
        scheme_admin: false,
        scheme_user: true,
      }
      const adminMembership = {
    ...
    it('Guest user with any membership', () => {
      const nonAdminMembership = {
        ...TestHelper.fakeTeamMember(guestUser.id, team.id),
        scheme_admin: false,
        scheme_user: true,
      }
      const adminMembership = {

...
function checkUserHasRole(
  ...
  return (
    ...
    (isSystemRole &&
      ...
      ((role === General.SYSTEM_USER_ROLE && userIsNotAdminOrGuest) || role !== General.SYSTEM_USER_ROLE)) ||
...
export function applyRolesFilters(
  ...
  const userIsNotAdminOrGuest = !(
    user.roles.includes(General.SYSTEM_ADMIN_ROLE) || user.roles.includes(General.SYSTEM_GUEST_ROLE)
  )
  ...
  return (
    filterRoles.length === 0 || filterRoles.some(checkUserHasRole.bind(this, user, userIsNotAdminOrGuest, membership))
  )
}
